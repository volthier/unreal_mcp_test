...
void AWeaponBase::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME(AWeaponBase, ChargeNormalized);
}
...
void AWeaponBase::AttachToOwnerMesh(USkeletalMeshComponent* OwnerMesh, FName SocketName)
{
	if (!OwnerMesh)
	{
		return;
...
FVector AWeaponBase::GetProjectileSpawnLocation() const
{
	return ProjectileSpawnSocket ? ProjectileSpawnSocket->GetComponentLocation() : GetActorLocation();
}

void AWeaponBase::SetChargeVisual(float NormalizedCharge)
{
...
void AWeaponBase::OnRep_ChargeNormalized()
{
	ApplyChargeMaterials(ChargeNormalized);
}

void AWeaponBase::ApplyChargeMaterials(float Normalized)
{
