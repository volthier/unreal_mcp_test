...
AVoltStrikerProjectile::AVoltStrikerProjectile()
{
...
void AVoltStrikerProjectile::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);
	DOREPLIFETIME(AVoltStrikerProjectile, bIsActive);
}

void AVoltStrikerProjectile::BeginPlay()
{
	Super::BeginPlay();
	CollisionSphere->OnComponentHit.AddDynamic(this, &AVoltStrikerProjectile::OnSphereHit);
}

void AVoltStrikerProjectile::LifeSpanExpired()
{
	DeactivateProjectile();
}

void AVoltStrikerProjectile::LaunchProjectile(const FVector& Direction, AActor* InInstigator, float InDamage, bool bInPiercing, float InAoERadius)
{
...
void AVoltStrikerProjectile::ResetProjectile()
{
	DeactivateProjectile();
}

void AVoltStrikerProjectile::OnSphereHit(UPrimitiveComponent* HitComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, FVector NormalImpulse, const FHitResult& Hit)
{
...
void AVoltStrikerProjectile::ApplyDamageAt(const FHitResult& Hit)
{
...
void AVoltStrikerProjectile::Multicast_PlayImpact_Implementation(FVector Location, FVector Normal)
{
	if (ImpactFX)
	{
...
void AVoltStrikerProjectile::DeactivateProjectile()
{
	bIsActive = false;
	SetLifeSpan(0.f);
