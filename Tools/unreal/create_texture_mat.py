#!/usr/bin/env python3
"""Create a material using a texture connected to BaseColor or EmissiveColor.
Usage: create_texture_mat.py <name> <texture_ref> <BaseColor|Emissive>"""
...
def call(session, ts, tool, args):
    ...
    return res
def main():
    name,tex,prop=sys.argv[1],sys.argv[2],sys.argv[3]
    mp="MP_EmissiveColor" if prop=="Emissive" else "MP_BaseColor"
    ...
    call(session=sid,ts=mt,tool="connect_to_output",args={"material":{"refPath":mpath},"expression":{"refPath":epath},"output_name":"RGB","material_property":mp})
    # metallic for metals
    call(session=sid,ts=mt,tool="recompile",args={"material_or_function":{"refPath":mpath}})
    print(f"{name} criado ({prop})")
