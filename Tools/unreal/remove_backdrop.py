#!/usr/bin/env python3
"""Remove the bad flat-box backdrop actors (Sky box + City pillars)."""
...
def main():
    ...
    # remove Sky box + City pillars (StaticMeshActors whose names are Sky/Sun/City)
    for name in ["Sky","Sun","City"]:
        acts=call(session=sid,ts=sms,tool="find_actors",args={"name":name,"tag":"","collision_channels":[]})
        for a in acts.get("returnValue",[]):
            p=a.get("refPath","")
            if "StaticMeshActor" in p:  # only remove the boxes, not SkyAtmosphere/SkyLight
                r=call(session=sid,ts=sms,tool="remove_from_scene",args={"actor":{"refPath":p}})
                print(f"removed {p[-22:]} ok={r.get('returnValue')}")
