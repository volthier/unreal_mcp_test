#!/usr/bin/env python3
"""Create an emissive solid-color material (color -> EmissiveColor) via UE MCP.
Usage: create_emissive_material.py <name> '<r,g,b>'"""
...
def call(session, ts, tool, args):
    ...
    return res
def main():
    name=sys.argv[1]; rgb=[float(x) for x in sys.argv[2].split(",")]
    ...
    mat=call(session=sid,ts=mt,tool="create_material",args={"folder_path":"/Game/Level","asset_name":name})
    ...
    if not epath: print("no expr"); return
    call(session=sid,ts=ot,tool="set_properties",args={"instance":{"refPath":epath},"values":json.dumps({"ParameterName":"Color","DefaultValue":{"R":rgb[0],"G":rgb[1],"B":rgb[2],"A":1.0}})})
    # connect to EmissiveColor so it glows regardless of lighting
