...
| `PROC-006` | **Consolidar o módulo quando o build roda com o editor aberto** | 🟠 | ✅ **ferramenta:** `Tools/unreal/consolidar_modulo.sh`. Com o editor aberto o UBT não escreve o dylib principal e cria iterações numeradas (`-0001`, `-0002`…). **Copiar a mais nova sobre a principal NÃO basta**: o arquivo carrega a própria identidade (`LC_ID_DYLIB`) apontando para o nome numerado, e o loader deixa de resolver o módulo. O sintoma é cruel: o editor abre, mas **commandlets rodam ZERO testes e o `pyscript` nem executa**, sem erro claro no log. A ferramenta copia a iteração, corrige a identidade com `install_name_tool -id`, re-assina e apaga as numeradas. O conserto limpo continua sendo fechar o editor e rodar o UBT. |
| `MIG-003` | Portar `Docs/ENGINEERING_PLAYBOOK_UNREAL.md` para cá e corrigir a versão citada (5.7 → 5.8) | 🟠 | ✅ **feito:** playbook portado para `Docs/`, versão corrigida para **5.8** e cabeçalho registrando a origem canônica e o vínculo com `Docs/Modelo_Interacao.md` (as seções 4 e 5 são a base do modelo de interação) |
...
| `PROC-006` | **Build com o editor aberto não chega no runtime dos testes** — o editor segura `libUnrealEditor-PloidrekRPG.dylib`, o UBT grava versões numeradas (`-0013`) e quem carrega o módulo é o arquivo antigo: os testes headless rodavam código velho **sem avisar** | 🔴 | Rotina conhecida e usada aqui: publicar a iteração mais nova por **inode novo** (`rm` + `cp`, nunca `cp -f`, que truncaria o inode mapeado pelo editor) antes de medir. Recentemente descoberto, não acontece de novo calado |
| `PROC-007` | **Teste em arquivo novo pode não registrar** — arquivo compilado fora do *unity* (feature de *adaptive unity*) entrou no link mas a automação não o listou até o teste morar num arquivo já registrado | 🟡 | Se um teste novo não aparecer em `Automation RunTests`, conferir se ele está no link e, se preciso, movê-lo para um arquivo de teste existente |
